...
def get_user_profile(request, user_id):
    ...
    try:
        ...
        # Проверяем кэш сначала для оптимизации
        ...
        if not user:
            ...
            # ВАЖНО: Не перезаписываем username, first_name, last_name если они уже есть
            try:
                user, created = TelegramUser.objects.get_or_create(
                    ...
                    defaults={
                        ...
                        # Не устанавливаем username, first_name, last_name в None - они могут быть обновлены через бота
                    }
                )
                logger.info(f"DEBUG: Пользователь {user_id} {'создан' if created else 'найден'}, username={user.username}, first_name={user.first_name}")
                # Кэшируем на 5 минут
            ...
            logger.debug(f"DEBUG: Пользователь {user_id} загружен из кэша")
...
def buy_subscription_with_balance(request):
    ...
    try:
        ...
        with transaction.atomic():
            ...
            PaymentService()._check_low_key_count(subscription_type)
            ...
            if subscription_type == 'week':
                ...
                subscription_expires_at = paid_at + timedelta(days=180)
            elif subscription_type == 'year':
