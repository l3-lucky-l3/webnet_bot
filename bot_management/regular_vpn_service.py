...
def _send_payment_success_notification_sync(payment: Payment, key: str):
    ...
    Отправляет одно сообщение с ключом и инструкцией
    """
    try:
...
⚡ ULTRA FAST VPN - Ключ активирован
...
📅 Действует до: {expires_date}"""
        ...
        keyboard = {
            "inline_keyboard": [
                [{"text": "⚡ Открыть ключ ULTRA FAST VPN", "url": key}],
                [{"text": "⬅️ Главное меню", "callback_data": "main_menu"},
                 {"text": "🛠 Поддержка", "url": "https://t.me/yamalube61"}]
            ]
        }
