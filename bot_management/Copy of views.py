...
def create_cryptobot_payment(request):
    ...
    try:
        ...
        subscription_names = {
            ...
            'fast_day': '1 день (Fast VPN)',
            'fast_month': '1 месяц (Fast VPN)',
            'fast_3months': '3 месяца (Fast VPN)',
            'fast_6months': '6 месяцев (Fast VPN)',
            'fast_year': '1 год (Fast VPN)',
        }
        vpn_label = "ULTRA FAST VPN" if vpn_type == 'regular' else ("Fast VPN" if vpn_type == 'fast' else "Night VPN")
    ...
    try:
        ...
        if platega_status_normalized == 'CONFIRMED' and payment.status != 'succeeded':
            ...
            if is_regular_vpn:
                ...
                logger.info(f"DEBUG: Платеж {payment.payment_id} - Fast VPN, обрабатываем через bypass API")
    ...
    try:
        ...
        if cryptobot_status_normalized == 'paid' and payment.status != 'succeeded':
            ...
            if is_regular_vpn:
                ...
                logger.info(f"Платеж {payment.payment_id} - Fast VPN, обрабатываем через bypass API")
                from .services import PaymentService
                payment_service = PaymentService()
                payment_service.confirm_payment(payment)
            else:
    ...
    try:
        ...
        if payment.status == 'succeeded':
            if payment.issued_key:
                ...
                logger.warning(f"Платеж {payment_id} в статусе succeeded но ключ не выдан. Выдаём ключ напрямую.")
                need_key_issue = True
        else:
            need_key_issue = False
        ...
        if is_regular_vpn:
            ...
            if result and result.get('success'):
                ...
                return JsonResponse({
                    'success': False,
                    'error': result.get('error', 'Ошибка генерации ключа') if result else 'Ошибка'
                }, status=500)
...
def get_prices_api(request):
    ...
    try:
        prices = {}
        for sub_type in ['month', '3months', '6months', 'year']:
