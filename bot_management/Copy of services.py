...
KEY_DELIVERY_MESSAGE_1 = """<b>🎉 Платеж подтвержден!</b>
...
📲 Установка и настройка Happ Plus
1. Скачайте приложение «Happ Plus» 
2. Копируем ссылку , которую мы отправили.
3. В правом верхнем углу нажмите «+» (плюсик).
4. Выберите опцию «Вставить из буфера обмена».
...
❗️ При глушении связи — выбирайте сервер с названием вашего оператор либо с припиской <b>ОБХОД ( тестируйте разные сервера найдете тот который работает лучше всего)</b>
❗️ Если интернет не глушат — используйте обычные сервера в самом низу списка под названием VPN , либо АВТО СЕРВЕРА
❗️ Чтобы установить на компьютер / телевизор нажмите на сам ключ и выберите нужное устройство далее скачайте предложенные приложения
...
KEY_DELIVERY_MESSAGE_2 = """<b>🌐 ЕСЛИ ПОДКЛЮЧЕНИЕ НЕ РАБОТАЕТ 🔧</b>

1. Нажмите на кнопку (как на правом скриншоте).
2. ⚠️ Избегайте неработающих серверов
Серверы со статусом «n/a» не работают. Не выбирайте их!
3. ✅ Выбирайте рабочий сервер
Кликайте на любой сервер, где отображается пинг (цифры в ms).
4. Правило простое: чем НИЖЕ цифра пинга — тем БЫСТРЕЕ будет ваше соединение! 🚀
...
class PaymentService:
    ...
    def confirm_payment(self, payment: Payment) -> bool:
        ...
        # Для Fast VPN используем bypass Remnawave API (1 squad)
        if vpn_type == 'fast':
            try:
                logger.info(f"DEBUG: Обработка платежа Fast VPN {payment.payment_id} через Remnawave Bypass API")
                from .night_vpn_fgn_service import process_fast_vpn_payment_sync

                result = process_fast_vpn_payment_sync(payment.payment_id)

                if result and result.get('success'):
                    logger.info(f"DEBUG: Платеж Fast VPN {payment.payment_id} успешно обработан")
                    payment.refresh_from_db()
                    self._save_profit(payment)
                    payment_confirmed = True
                    ...
                    logger.error(f"DEBUG: Ошибка обработки платежа Fast VPN {payment.payment_id}: {error_msg}")
                    payment.status = 'succeeded'
                    payment.paid_at = timezone.now()
                    payment.save()
                    payment_confirmed = True
                    self._send_fast_vpn_key_notification(payment)
                return payment_confirmed
            except Exception as e:
                logger.error(f"DEBUG: Исключение при обработке платежа Fast VPN {payment.payment_id}: {e}")
                import traceback
                logger.error(f"DEBUG: Traceback: {traceback.format_exc()}")
                return False

        # Для Night VPN (ОБХОД глушилок) используем Remnawave Bypass API
        ...
        if use_bypass_api:
            try:
                ...
                if result and result.get('success'):
                    ...
                    logger.warning(f"DEBUG: Пробуем старый метод с пулом ключей для платежа {payment.payment_id}")
                    payment_confirmed = self._confirm_payment_with_key_pool(payment)
                return payment_confirmed
            except Exception as e:
        ...
        try:
            ...
            if not bypass_uuid:
                ...
                if latest_payment:
                    ...
                    logger.warning("Пробуем создать ключ как обычную покупку")
                    return self._confirm_payment_with_key_pool(payment)
...
🔑 Ваш ключ: {original_payment.issued_key}"""
    ...
    def _confirm_renewal_payment_fast(self, payment: Payment, original_payment: Payment, months: int) -> bool:
        """Продление Fast VPN через Remnawave Bypass API."""
        try:
            from .night_vpn_fgn_service import extend_bypass_subscription
            ...
            if not bypass_uuid:
                logger.error(f"Нет bypass_remnawave_uuid у Fast VPN платежа {original_payment.payment_id}")
                return False

            days_map = {'day': 1, 'month': 30, '3months': 90, '6months': 180, 'year': 365}
            duration_days = days_map.get(payment.subscription_type, 30)

            extended = extend_bypass_subscription(original_payment, duration_days)
            if not extended:
                logger.error(f"Не удалось продлить Fast VPN подписку для платежа {payment.payment_id}")
                return False
...
🔑 Ваш ключ: {original_payment.issued_key}"""
            ...
            logger.error(f"Ошибка продления Fast VPN: {e}")
            import traceback
            logger.error(traceback.format_exc())
            return False
...
Подписка действует до {payment.subscription_expires_at.strftime('%d.%m.%Y') if payment.subscription_expires_at else '—'}."""
    ...
    def _issue_regular_vpn_key_for_night_payment(self, payment: Payment, night_key: str = None) -> bool:
        """
        Выдаёт ключ обычного VPN при покупке Night VPN (Обход глушилок).
...
"""
    ...
    def _send_night_vpn_key_notification(self, payment: Payment, key_value: str = None):
        """Отправляет уведомление с ключом ОБХОД глушилок пользователю"""
        try:
            if not key_value:
...
🛡️ <b>ОБХОД глушилок + VPN - Ключ активирован</b>
...
📲 <b>Установка и настройка Happ</b>
...
🌍 <b>Выбор сервера от ГЛУШИЛОК:</b>
❗️ При глушении связи — выбирайте сервер с названием вашего оператора либо с припиской <b>ОБХОД</b> (тестируйте разные сервера, найдете тот который работает лучше всего)
❗️ Если интернет не глушат — используйте обычные сервера в самом низу списка под названием VPN, либо АВТО СЕРВЕРА
...
· При нарушении этого правила доступ может быть заблокирован без возможности возврата средств"""
            ...
            import json
            keyboard_buttons = [
                [{"text": "🛡️ Открыть ключ ОБХОД глушилок", "url": key_value}],
            ...
            troubleshooting_msg = """🌐 <b>ЕСЛИ ПОДКЛЮЧЕНИЕ НЕ РАБОТАЕТ</b> 🔧

1. Нажмите на кнопку (как на правом скриншоте).
2. ⚠️ Избегайте неработающих серверов — серверы со статусом «n/a» не работают. Не выбирайте их!
3. ✅ Выбирайте рабочий сервер — кликайте на любой сервер, где отображается пинг (цифры в ms).
4. Правило простое: чем НИЖЕ цифра пинга — тем БЫСТРЕЕ будет ваше соединение! 🚀
...
Выбирайте ТОЛЬКО серверы с пометкой «ОБХОД» либо с названием вашего оператора."""
            ...
            photo_path1 = Path("images/instruction3_1.jpg")
            photo_path2 = Path("images/instruction3_2.jpg")
            if photo_path1.exists() and photo_path2.exists():
                from config import BOT_TOKEN
                import requests as req
                tg_url = f"https://api.telegram.org/bot{BOT_TOKEN}/sendMediaGroup"
                with open(photo_path1, 'rb') as f1, open(photo_path2, 'rb') as f2:
                    files = {'photo1': f1, 'photo2': f2}
                    media = [
                        {'type': 'photo', 'media': 'attach://photo1', 'caption': troubleshooting_msg, 'parse_mode': 'HTML'},
                        {'type': 'photo', 'media': 'attach://photo2'}
                    ]
                    req.post(tg_url, files=files, data={'chat_id': payment.user.user_id, 'media': json.dumps(media)}, timeout=10)
                time.sleep(1)
                kb_url = f"https://api.telegram.org/bot{BOT_TOKEN}/sendMessage"
                req.post(kb_url, data={
                    'chat_id': payment.user.user_id,
                    'text': '💡 <b>Если возникли проблемы с подключением, обратитесь к менеджеру</b>',
                    'parse_mode': 'HTML',
    ...
    def _send_fast_vpn_key_notification(self, payment: Payment, key_value: str = None):
        """Отправляет уведомление с ключом Fast VPN пользователю"""
        try:
            if not key_value:
                key_value = payment.issued_key
            ...
            import json
            keyboard_buttons = [
                [{"text": "🚀 Открыть ключ Fast VPN", "url": key_value}],
                [{"text": "⬅️ Главное меню", "callback_data": "main_menu"}],
            ]
            keyboard = {"inline_keyboard": keyboard_buttons}
...
<i>Спасибо за покупку! 🚀</i>"""
            ...
            self._send_telegram_message_sync(payment.user.user_id, message)
            logger.info(f"Уведомление Fast VPN отправлено пользователю {payment.user.user_id}")

        except Exception as e:
            logger.error(f"Ошибка отправки уведомления Fast VPN пользователю {payment.user.user_id}: {e}")
    ...
    async def _notify_user_payment_confirmed(self, payment: Payment, key: SubscriptionKey):
        ...
        try:
            ...
            text1 = KEY_DELIVERY_MESSAGE_1.format(issued_key=html.escape(key.key_value))
            await self.bot.send_message(payment.user.user_id, text1, parse_mode="HTML", reply_markup=keyboard)
            await asyncio.sleep(2)

            # Сообщение 2: только 2 картинки с текстом «ЕСЛИ ПОДКЛЮЧЕНИЕ НЕ РАБОТАЕТ»
            photo_path1 = Path("images/instruction3_1.jpg")
            photo_path2 = Path("images/instruction3_2.jpg")
            if photo_path1.exists() and photo_path2.exists():
                media = [
                    InputMediaPhoto(media=FSInputFile(photo_path1), caption=KEY_DELIVERY_MESSAGE_2, parse_mode="HTML"),
                    InputMediaPhoto(media=FSInputFile(photo_path2)),
                ]
                await self.bot.send_media_group(payment.user.user_id, media)
                # Отправляем кнопки отдельным сообщением после фото
                await asyncio.sleep(1)
                await self.bot.send_message(payment.user.user_id, "💡 <b>Если возникли проблемы с подключением, обратитесь к менеджеру</b>", parse_mode="HTML", reply_markup=keyboard)
            else:
    ...
    def _notify_user_payment_confirmed_sync(self, payment: Payment, key: SubscriptionKey):
        ...
        try:
            ...
            # Сообщение 2: только 2 фото с текстом «ЕСЛИ ПОДКЛЮЧЕНИЕ НЕ РАБОТАЕТ»
            photo_path1 = Path("images/instruction3_1.jpg")
            photo_path2 = Path("images/instruction3_2.jpg")
            if photo_path1.exists() and photo_path2.exists():
                tg_url = f"https://api.telegram.org/bot{BOT_TOKEN}/sendMediaGroup"
                with open(photo_path1, 'rb') as f1, open(photo_path2, 'rb') as f2:
                    files = {'photo1': f1, 'photo2': f2}
                    media = [
                        {'type': 'photo', 'media': 'attach://photo1', 'caption': KEY_DELIVERY_MESSAGE_2, 'parse_mode': 'HTML'},
                        {'type': 'photo', 'media': 'attach://photo2'}
                    ]
                    requests.post(tg_url, files=files, data={'chat_id': payment.user.user_id, 'media': json.dumps(media)}, timeout=10)
                # Отправляем кнопки отдельным сообщением после фото
                import time
                time.sleep(1)
                msg_url = f"https://api.telegram.org/bot{BOT_TOKEN}/sendMessage"
                requests.post(msg_url, data={'chat_id': payment.user.user_id, 'text': '💡 <b>Если возникли проблемы с подключением, обратитесь к менеджеру</b>', 'parse_mode': 'HTML', 'reply_markup': json.dumps(keyboard)}, timeout=5)
            else:
