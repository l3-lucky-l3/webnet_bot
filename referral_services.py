...
class ReferralService:
    ...
    def process_referral(self, referrer_code: str, referred_user_id: int) -> dict:
        ...
        try:
            ...
            return {
                'success': True,
                'referrer': referrer,
                'message': f'Вы приглашены пользователем @{referrer.username or referrer.first_name}'
            }
    ...
    def get_referral_stats_sync(self, user_id: int) -> dict:
        ...
        try:
            ...
            referrals = Referral.objects.filter(referrer=user, is_active=True)
            ...
            # Общая выручка от покупок рефералов - учитываем все успешные платежи
            from django.db.models import Sum
            total_revenue = 0
            if referred_user_ids:
    ...
    async def process_referral_purchase(self, user_id: int, payment: Payment) -> dict:
        ...
        try:
            ...
            await sync_to_async(ReferralBalanceTransaction.objects.create)(
                ...
                description=f'Реферальная награда 20% за покупку пользователя {payment.user.username or payment.user.user_id}'
            )
            
            # Уведомляем реферера - только для платных подписок, исключая trial
            if self.bot and payment.subscription_type != 'trial':
                try:
                    message = f"""
🎉 <b>Реферальная награда!</b>

💰 <b>Начислено на реферальный баланс:</b> {reward_amount} ₽
👤 <b>За покупку:</b> @{payment.user.username or payment.user.first_name}
...
"""
    ...
    def process_referral_purchase_sync(self, user_id: int, payment) -> dict:
        ...
        try:
            ...
            # Уведомляем реферера (если есть бот) - только для платных подписок, исключая trial
            if self.bot and payment.subscription_type != 'trial':
                try:
                    ...
                    # Имя пользователя
                    user_name = payment.user.username or payment.user.first_name or f"ID{payment.user.user_id}"
