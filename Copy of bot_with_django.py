...
from config import BOT_TOKEN, ADMIN_IDS, PRICES, VIDEO_FILE_ID, VIDEO_URL, VIDEO_POST_URL, OPERATORS, SUPPORT_GROUP_ID, REQUIRED_CHANNEL, REQUIRED_CHANNEL_ID, DISABLE_PHOTOS, REGULAR_VPN_PRICES, DJANGO_API_URL
...
"""
    ...
    keyboard.extend([
        ...
        [InlineKeyboardButton(text="📌 Прокси для Telegram", callback_data="tg_proxy")],
...
async def cmd_start(message: Message, command: CommandObject = None):
    ...
    try:
        # Используем ID канала (более надежно)
        ...
        # Если ошибка проверки, пропускаем проверку, но логируем
        logging.warning(f"Ошибка проверки подписки на канал {REQUIRED_CHANNEL_ID}: {e}")
        logging.warning("Проверка подписки пропущена. Убедитесь, что бот является администратором канала.")
    
    # Если требуется подписка, показываем сообщение
...
✅ <b>После подписки нажмите кнопку "Проверить подписку"</b>

<i>Это необходимо для получения актуальных новостей и обновлений</i>
{referral_message}
"""
...
🎁 <b>Ваш ключ:</b> <code>{issued_key}</code>
...
"""
                    ...
                    logger.error(f"Ошибка получения данных платежа {payment_id}: {e}")
            
            # Стандартное сообщение для ОБХОД глушилок + VPN
...
""", parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
    ...
    welcome_text = f"""
<b>Глушат связь?</b>{referral_message}
...
async def check_subscription(callback: CallbackQuery):
    ...
    try:
        ...
        if channel_member.status in ['member', 'administrator', 'creator']:
            ...
            # Вызываем главное меню напрямую
...
async def choose_subscription(callback: CallbackQuery, state: FSMContext):
    """Обработчик выбора подписки для ОБХОД глушилок + VPN и Обычного VPN"""
    # Определяем тип подписки и VPN
    ...
    # Маппинг callback_data на тип подписки
    sub_type_map = {
        "sub_month": "month",
        "sub_3months": "3months",
        "sub_6months": "6months",
    ...
    # Проверяем подписку на канал перед оплатой
    try:
        # Используем ID канала (более надежно)
...
📢 <b>Для покупки подписки необходимо подписаться на наш канал:</b>
👉 {REQUIRED_CHANNEL}
...
""", parse_mode="HTML", reply_markup=kb)
        ...
        logging.warning(f"Ошибка проверки подписки на канал {REQUIRED_CHANNEL_ID}: {e}")
        # Пропускаем проверку при ошибке (бот не админ или другая ошибка)
        pass
...
async def check_payment_status(callback: CallbackQuery):
    ...
    if DJANGO_INTEGRATION:
        try:
            ...
            # Если ключ уже выдан — не дублируем
            if payment_status == 'succeeded' and issued_key:
                await callback.answer("✅ Платеж уже подтвержден! Ключ выдан.", show_alert=True)
                return
                ...
                async with session.get(api_url) as response:
                    if response.status == 200:
                        ...
                        is_paid = (payment_type == 'cryptobot' and status_normalized == 'paid') or \
                                  (payment_type == 'platega' and status_normalized == 'CONFIRMED')

                        if is_paid and payment_status == 'succeeded':
                            if issued_key:
                                await callback.answer("✅ Платеж уже подтвержден! Ключ выдан.", show_alert=True)
                            else:
                                await callback.answer("⏳ Ключ еще не выдан. Обрабатывается...", show_alert=True)
                        elif is_paid and payment_status != 'succeeded':
                            await callback.answer("⏳ Платеж подтвержден, обрабатывается...", show_alert=True)
...
""", parse_mode="HTML")
    ...
    if DJANGO_INTEGRATION:
        ...
        if django_chat_id:
            ...
            if SUPPORT_GROUP_ID:
                try:
                    admin_url = f"http://127.0.0.1:8188/bot_management/support/{chat_id}/"
                    group_message = f"""
...
"""
    ...
    about_kb = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [
        InlineKeyboardButton(text="📋 Пользовательское соглашение", url="https://telegra.ph/Polzovatelskoe-soglashenie-12-14-17"),
        ]
    ])
...
async def pay_with_sbp(callback: CallbackQuery):
    """Обработка оплаты по СБП (ручная)"""
    user_id = callback.from_user.id

    # Извлекаем тип подписки и vpn_type
    # Формат: pay_sbp_{sub_type}_{vpn_type} или pay_sbp_{sub_type} (старый формат)
    ...
    sub_name = sub_names.get(subscription_type, 'Подписка')
    vpn_label = "🌍 Обычный VPN" if vpn_type == 'regular' else "🛡️ ОБХОД глушилок + VPN"

    # Создаем платеж
    if DJANGO_INTEGRATION:
        if vpn_type == 'regular':
            # Для Обычного VPN создаем платеж с vpn_type='regular'
            ...
            if payment_data:
                keyboard = InlineKeyboardMarkup(inline_keyboard=[
                    [InlineKeyboardButton(text="💳 Оплатить", url=payment_data['confirmation_url'])],
                    [InlineKeyboardButton(text="🔄 Проверить статус", callback_data=f"check_payment_{payment_data['payment_id']}")],
                    [InlineKeyboardButton(text="⬅️ Назад", callback_data="catalog_regular_vpn")]
                ])

                await callback.message.answer(f"""
💸 <b>Оплата подписки по СБП</b>
...
""", parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
    [InlineKeyboardButton(text="🔄 Попробовать снова", callback_data=callback.data)],
    [InlineKeyboardButton(text="⬅️ Каталог", callback_data="catalog_regular_vpn")]
]))
        else:
            # Для ОБХОД глушилок + VPN используем старый метод
            payment_data = await create_platega_payment(
                user_id=user_id,
                subscription_type=subscription_type,
                amount=amount,
...
""", parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
    [InlineKeyboardButton(text="🔄 Попробовать снова", callback_data=callback.data)],
    [InlineKeyboardButton(text="⬅️ Каталог", callback_data="catalog_night_vpn")]
]))
    else:
...
async def pay_with_cryptobot(callback: CallbackQuery):
    ...
    if DJANGO_INTEGRATION:
        ...
        crypto_subscription_type = f'regular_{subscription_type}' if vpn_type == 'regular' else subscription_type
        ...
        if payment_data:
            ...
            back_callback = "catalog_regular_vpn" if vpn_type == 'regular' else "catalog_night_vpn"
...
""", parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
    [InlineKeyboardButton(text="🔄 Попробовать снова", callback_data=callback.data)],
    [InlineKeyboardButton(text="⬅️ Каталог", callback_data="catalog_regular_vpn" if vpn_type == 'regular' else "catalog_night_vpn")]
]))
    else:
        await callback.answer("❌ Сервис оплаты недоступен", show_alert=True)
    ...
    if DJANGO_INTEGRATION:
        if vpn_type == 'regular':
            ...
            payment_data = await create_platega_payment(
                ...
                amount=amount,
                return_url=None,
                payment_method=11,  # Банковская карта
                vpn_type='regular'  # Тип VPN
            )
...
• Работает когда глушат интернет
• Стабильное соединение
• Поддержка 24/7

🌍 <b>Обычный VPN</b>
• Классический VPN без ограничений
• Поддержка 24/7
...
"""
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🌍 Обычный VPN", callback_data="catalog_regular_vpn")],
        [InlineKeyboardButton(text="⬅️ Главное меню", callback_data="main_menu")]
    ])
...
async def show_regular_vpn_catalog(callback: CallbackQuery):
    """Показать каталог Обычного VPN (Remnawave)"""
    # Цены из config.py
...
"""
    ...
    # Проверяем статус пробных ключей для обоих типов VPN
    ...
    if not regular_trial_used:
        keyboard.append([InlineKeyboardButton(text="🌍Пробный Обычный VPN", callback_data="trial_key_info_regular")])
    
    # Если оба пробных ключа использованы, показываем сообщение
    if night_trial_used and regular_trial_used:
        menu_text = """
🔑 <b>Получить VPN ключ</b>
...
• Попробовать Обычный VPN (доступен пробный ключ)
"""
        kb = InlineKeyboardMarkup(inline_keyboard=[
...
<i>После получения активируйте ключ сразу!</i>
"""
        kb = InlineKeyboardMarkup(inline_keyboard=[
...
💳 <b>Доступные варианты:</b>
• Приобрести полную подписку
• Попробовать ОБХОД глушилок + VPN (доступен пробный ключ)
"""
        kb = InlineKeyboardMarkup(inline_keyboard=[
...
async def issue_trial_key_for_vpn_type(callback, user_id, vpn_type, vpn_label):
    """Общая функция для выдачи пробного ключа (два сообщения как при покупке)"""
    if DJANGO_INTEGRATION:
        try:
            import aiohttp
            import json

            api_url = f'{DJANGO_API_URL}/bot_management/api/user/{user_id}/issue_trial_key/'
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        if data.get('success'):
                            issued_key = data.get('issued_key')
                            expires_at = data.get('expires_at', 'неизвестно')
                            regular_vpn_key = data.get('regular_vpn_key')

                            # Формируем текст сообщения
                            if vpn_type == 'night' and regular_vpn_key:
                                # Комбинированное сообщение для Обход + VPN
                                message1_text = f"""
🎉 <b>Пробный ключ активирован!</b>

🛡️ <b>Ваш ключ ОБХОД глушилок:</b>
{issued_key}

🌍 <b>Ваш ключ обычного VPN:</b>
{regular_vpn_key}

<i>Спасибо за интерес к нашему сервису! 🚀</i>

📅 <b>Оба ключа действительны до:</b> {expires_at}
⏰ <b>Срок действия:</b> 24 часа

<b>🔑 Как подключить устройство?</b>

📲 <b>Инструкция по настройке:</b>
1. Нажмите на кнопку ключа ниже(или ссылку выше) — он откроется в браузере
2. Приложение автоматически предложит добавить подписку
3. Готово!

<b>⚠️ Условия использования</b>
· Доступ на 1 устройство для каждого ключа
· Пробный период предоставляется только один раз
· При нарушении правил — бан без возможности повторной выдачи

<i>📌 Ниже отправлена подробная инструкция по выбору серверов и настройке!</i>
"""
                            else:
                                # Обычное сообщение для одного ключа
                                message1_text = f"""
🎉 <b>Пробный ключ активирован!</b>
✅ <b>Ваша пробная подписка подключена</b>
🎁 <b>Ваш ключ:</b> <code>{issued_key}</code>
...
📲 <b>Инструкция по настройке:</b>
1. Перейдите по ссылке выданной выше
2. Выберите приложение удобное вам
3. Установите его(Пропустите данный шаг если уже скачано)
...
"""
                                ...
                                # Отправляем сначала сообщение с инструкцией
                                kb_instruction = InlineKeyboardMarkup(inline_keyboard=[
                                    [InlineKeyboardButton(text="🏠 Главное меню", callback_data="main_menu")],
                                    [InlineKeyboardButton(text="💬 Написать менеджеру", url="https://t.me/yamalube61")]
                                ])
                                ...
                                # Ждём 2-3 секунды перед отправкой сообщения с ключами
                                import asyncio
                                await asyncio.sleep(2.5)

                                # Формируем кнопки для сообщения с ключами
                                kb_buttons = []
                                if vpn_type == 'night' and regular_vpn_key:
                                    kb_buttons.append([InlineKeyboardButton(text="🛡️ Открыть ключ ОБХОД", url=issued_key)])
                                    kb_buttons.append([InlineKeyboardButton(text="🌍 Открыть обычный VPN", url=regular_vpn_key)])
                                else:
                                    kb_buttons.append([InlineKeyboardButton(text="📋 Подключить подписку", url=f"{issued_key}")])
                                kb_buttons.append([
                                    InlineKeyboardButton(text="🏠 Главное меню", callback_data="main_menu"),
                                    InlineKeyboardButton(text="💬 Написать менеджеру", url="https://t.me/yamalube61")
                                ])
                                
                                kb1 = InlineKeyboardMarkup(inline_keyboard=kb_buttons)
        ...
        # Ошибка выдачи ключа
        error_text = f"""
❌ <b>Ошибка получения пробного ключа</b>
                    ...
                    if response.status == 200:
                        ...
                        if data.get('success') and data.get('keys'):
                            ...
                            if regular_keys:
                                ...
                                await send_or_edit_message_with_photo(callback, "📭 <b>У вас нет ключей Обычного VPN</b>", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                                    [InlineKeyboardButton(text="⬅️ Назад", callback_data="my_keys")],
                                    [InlineKeyboardButton(text="⬅️ Главное меню", callback_data="main_menu")]
                                ]), edit_message=True, image_name="my_keys.png")
                        else:
                    ...
                    if response.status == 200:
                        ...
                        if data.get('success') and data.get('keys'):
                            ...
                            if night_keys:
                                ...
                                await send_or_edit_message_with_photo(callback, "📭 <b>У вас нет ключей ОБХОД глушилок + VPN</b>", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                                    [InlineKeyboardButton(text="⬅️ Назад", callback_data="my_keys")],
                                    [InlineKeyboardButton(text="⬅️ Главное меню", callback_data="main_menu")]
                                ]), edit_message=True, image_name="my_keys.png")
                        else:
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(api_url) as response:
                ...
                from config import REQUIRED_CHANNEL_ID, REQUIRED_CHANNEL, CRYPTOBOT_API_TOKEN
                try:
                    channel_member = await bot.get_chat_member(REQUIRED_CHANNEL_ID, callback.from_user.id)
                    if channel_member.status not in ['member', 'administrator', 'creator']:
                        channel_username = REQUIRED_CHANNEL.replace('@', '')
                        ...
                        await callback.message.answer(
                            ...
                            f"📢 <b>Для продления необходимо подписаться на наш канал:</b>\n"
                            f"👉 {REQUIRED_CHANNEL}\n\n"
                            f"✅ <b>После подписки нажмите 'Проверить подписку'</b>",
                            parse_mode="HTML", reply_markup=kb
                        )
                        await callback.answer("❌ Требуется подписка на канал", show_alert=True)
                        return
...
async def process_renewal_payment(callback: CallbackQuery, payment_id: int, method: str):
    """Создает платеж для продления через Platega"""
    user_id = callback.from_user.id
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(api_url) as response:
                ...
                # Создаем платеж через Django API
                ...
                async with aiohttp.ClientSession() as session2:
                    async with session2.post(create_url, json=payment_data) as resp2:
                        ...
                        # CryptoBot возвращает {'status': 'success'} а Platega {'success': true}
...
async def check_renewal_payment(callback: CallbackQuery):
    ...
    user_id = callback.from_user.id
    payment_id = callback.data.split(":")[1]  # payment_id оригинального платежа
    ...
    try:
        ...
        # Получаем все платежи пользователя и ищем последний pending для продления
        keys_url = f'{DJANGO_API_URL}/bot_management/api/users/{user_id}/keys/'
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(keys_url) as response:
                ...
                # Ищем последний pending платеж продления
                latest_renewal_payment = None
                for key in data.get('keys', []):
                    if key.get('status') == 'pending' and key.get('is_renewal'):
                        latest_renewal_payment = key
                        break

                if not latest_renewal_payment:
                    # Если не нашли в keys, пробуем через API статуса платежа
                    await callback.answer("⏳ Платёж не найден. Если оплатили — подождите 1-2 мин.", show_alert=True)
                    return

                renewal_payment_id = latest_renewal_payment.get('payment_id')
                platega_id = latest_renewal_payment.get('platega_transaction_id')
                cryptobot_id = latest_renewal_payment.get('cryptobot_invoice_id')
                payment_status = latest_renewal_payment.get('status', 'pending')
                issued_key = latest_renewal_payment.get('key_value')
...
📋 <b>Платеж:</b> #{renewal_payment_id}
✅ <b>Статус:</b> Оплачено

🔑 <b>Ключ:</b> <code>{issued_key}</code>

🔄 Подписка продлена. Проверьте в "Мои ключи".""",
                        parse_mode="HTML"
                    )
                ...
                if cryptobot_id:
                    check_url = f'{DJANGO_API_URL}/bot_management/api/payments/{renewal_payment_id}/cryptobot-status/'
                elif platega_id:
                    check_url = f'{DJANGO_API_URL}/bot_management/api/payments/{renewal_payment_id}/platega-status/'
                else:
                    await callback.answer("❌ Неизвестный тип платежа", show_alert=True)
                    return

                async with aiohttp.ClientSession() as session2:
                    async with session2.get(check_url) as resp2:
                        if resp2.status != 200:
                            await callback.answer("❌ Ошибка проверки статуса", show_alert=True)
                            return
...
🔄 Подписка продлена. Проверьте в "Мои ключи".""",
                            ...
                            await callback.answer("⏳ Оплата ещё не подтверждена. Подождите и попробуйте снова.", show_alert=True)
                        else:
                            await callback.answer(f"❌ Платеж отменён или ошибка", show_alert=True)
...
async def handle_custom_deposit_amount(message: Message, state: FSMContext):
    ...
    current_state = await state.get_state()
    logging.info(f"DEBUG handle_custom_deposit_amount: состояние = {current_state}, BroadcastState.waiting_message = {BroadcastState.waiting_message}")
    if current_state == BroadcastState.waiting_message or current_state == BroadcastState.waiting_confirmation:
        logging.info(f"DEBUG: Пропуск обработки числа, так как пользователь в состоянии рассылки")
        return
    
    # Пропускаем, если пользователь в состоянии загрузки ключей
    if current_state == UploadKeysState.waiting_keys:
        logging.info(f"DEBUG: Пропуск обработки, так как пользователь в состоянии загрузки ключей")
        return
...
async def handle_unknown_message(message: Message, state: FSMContext):
    ...
    # Пропускаем сообщения, если пользователь находится в состоянии загрузки ключей
    if current_state == UploadKeysState.waiting_keys:
        return
...
"""
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🎁 Пробная (1 день)", callback_data="upload_keys_trial")],
            ...
            try:
                async with session.get(api_url, timeout=aiohttp.ClientTimeout(total=15)) as response:
                    if response.status != 200:
                        ...
                        await callback.message.answer(
                            "❌ Ошибка загрузки списка рефереров. Проверьте, что Django-сервер запущен (порт 8188)."
                        )
                        return
                ...
                await callback.message.answer("❌ Не удалось подключиться к серверу. Убедитесь, что Django запущен (порт 8188).")
...
async def admin_payment_confirm_handler(callback: CallbackQuery):
    ...
    try:
        ...
        api_url = f'{DJANGO_API_URL}/bot_management/api/payments/confirm/'
